...
class V4ViewPagerExt extends ViewGroup {
    ...
    private static final String TAG = LzViewPager.class.getSimpleName();
    private static final boolean DEBUG = false;

    private static final boolean USE_CACHE = false;

    private static final int DEFAULT_OFFSCREEN_PAGES = 0; // 原生默认是1
    private static final int MAX_SETTLE_DURATION = 600; // 默认的ViewPager翻页时长由多个网页计算，最大为600 ms
    private int mScrollDuration = -1; // 自定义ViewPager翻页时长
    ...
    public V4ViewPagerExt(Context context) {
        super(context);
        initViewPager();
    }

    public V4ViewPagerExt(Context context, AttributeSet attrs) {
        super(context, attrs);
        initViewPager();
    }
...
//            mAdapter.registerDataSetObserver(mObserver);
    ...
    void smoothScrollTo(int x, int y, int velocity) {
        ...
        if (mScrollDuration == -1) {
            ...
            // 自定义翻页时长
            mScroller.startScroll(sx, sy, dx, dy, mScrollDuration);
        }
